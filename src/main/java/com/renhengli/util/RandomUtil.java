...
public class RandomUtil {
	 ...
	 *            随机字符串长度
	 * @param type
	 *            (up:大写 lower：小写 ignore 随机)
	 * @return 随机字符串  例如：GU55-QKCQ-0JBP-DYZ8
	 */
	public static String generateString(String type, int... lengths) {
		...
		String result = "";
		if ("up".equals(type)) {
			result = sb.toString().substring(0, sb.length() - 1).toUpperCase();
		} else if ("lower".equals(type)) {
			result = sb.toString().substring(0, sb.length() - 1).toLowerCase();
		} else {
	...
	public static void main(String[] args) {
		System.out.println("返回一个定长的随机字符串(只包含大小写字母、数字):" + generateString("up",10));
		...
		System.out.println("每次生成的len位数都不相同:" + getNotSimple(in, 5));
	}
}